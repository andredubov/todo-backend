...
func TestHandler_getItemByID(t *testing.T) {
	...
	tests := []test{
		{
			...
			mockBehavior: func(s *mock_service.MockTodoItem, args args) {
				todoItem := domain.TodoItem{Id: 1, Title: "title1", Description: "description1", Done: true}
				s.EXPECT().GetById(gomock.Any(), args.userId, args.todoItemId).Return(todoItem, nil)
			},
			expectedStatusCode:   http.StatusOK,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			w, endpoint := httptest.NewRecorder(), fmt.Sprintf("/api/items/%d", test.input.todoItemId)
			r := httptest.NewRequest(http.MethodGet, endpoint, bytes.NewBufferString(test.inputRequestBody))
			r.Header.Set(authorizationHeader, bearer+" "+token) // set jwt token
			router.ServeHTTP(w, r)                              // perforn request
...
func TestHandler_updateItemByID(t *testing.T) {
	...
	tests := []test{
		{
			...
			jwtTTL:           time.Duration(5 * time.Minute),
			delay:            time.Duration(0 * time.Millisecond),
			inputRequestBody: `{"title": "new title", "description": "new description", "done": true}`,
			input: args{
				userId:     1,
				todoItemId: 2,
				updateTodoItemInput: domain.UpdateTodoItemInput{
			...
			mockBehavior: func(s *mock_service.MockTodoItem, args args) {
				s.EXPECT().Update(gomock.Any(), args.userId, args.todoItemId, args.updateTodoItemInput).Return(nil)
			},
			expectedStatusCode:   http.StatusOK,
		...
		},
		{
			...
			jwtTTL:           time.Duration(5 * time.Minute),
			delay:            time.Duration(0 * time.Millisecond),
			inputRequestBody: `{"description": "new description", "done": true}`,
			input: args{
				userId:     1,
				todoItemId: 2,
				updateTodoItemInput: domain.UpdateTodoItemInput{
		...
		},
		{
			...
			jwtTTL:           time.Duration(5 * time.Minute),
			delay:            time.Duration(0 * time.Millisecond),
			inputRequestBody: `{"title": "new title", "done": true}`,
			input: args{
				userId:     1,
				todoItemId: 2,
				updateTodoItemInput: domain.UpdateTodoItemInput{
