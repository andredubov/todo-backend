...
func TestHandler_deleteListByID(t *testing.T) {
	...
	tests := []test{
		{
			...
			mockBehavior: func(s *mock_service.MockTodoList, args args) {
				s.EXPECT().Delete(gomock.Any(), args.userId, args.todoListId).Return(nil)
			},
			expectedStatusCode:   http.StatusOK,
			expectedResponseBody: "{\"status\":\"success\"}\n",
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			w := httptest.NewRecorder()
			r := httptest.NewRequest(http.MethodDelete, fmt.Sprintf("/api/lists/%d", test.input.todoListId), bytes.NewBufferString(test.inputRequestBody))
			r.Header.Set(authorizationHeader, bearer+" "+token) // set jwt token
			router.ServeHTTP(w, r)                              // perforn request
